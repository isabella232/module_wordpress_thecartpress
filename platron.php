...
if ( ! class_exists( 'TCPSkeletonLoader' ) ) {
	...
	class TheCartPress_Platron_Loader {
			public static $plugin_title = 'TheCartPress - Platron payment gateway';
			public static $plugin_description = 'Integrates Platron payment gateway with yout TheCartPress shop.';
	        /**
	...
	function tcp_load_platron_plugin() {
		...
		class PlatronForTheCartPress extends TCP_Plugin {
			...
			function getFields() {
				$fields = array(
					'platron_merchant_id' => array(
						'title' => __('Platron Merchant ID', 'tcp-platron'),
						'type' => 'text',
						'description' => __('Type in your merchant ID from <a href="https://platron.ru/admin/merchants.php">Platron</a>.', 'tcp-platron')
					),	
					'platron_secret_key' => array(
						'title' => __('Secret key', 'tcp-platron'),
						'type' => 'text',
						'description' => __('Secret key from <a href="https://platron.ru/admin/merchants.php">Platron</a>.', 'tcp-platron')
					),
					...
					'platron_logo' => array(
						...
						'default' => 'https://www.platron.ru/integration/modules_images/platron.png',
					),
				);
			...
			function showPayForm( $instance, $shippingCountry, $shoppingCart, $order_id ) {
				...
				echo '<form id="platron_payment_form" action="https://www.platron.ru/payment.php" method="post">';
			...
			function tcp_platron_ipn() {
				...
				if(!isset($arrRequest['pg_result'])){
					...
					// Transaction status
					$additional = 'Platron transaction # ' . $arrRequest['pg_payment_id'];
